...
export function buildSearchIndex(
  ...
  const push = (type: ComponentType, name: string): void => {
    const key = `${type}:${name}`;
  ...
  for (const l of graph.lwcs) push("lwc", l.fullyQualifiedName);
  for (const o of graph.objects) push("object", o.fullyQualifiedName);
  for (const f of graph.flows) push("flow", f.fullyQualifiedName);
